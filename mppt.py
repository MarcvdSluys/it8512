...
set_volts_delta_low = 0.32  # Lowest voltage change that makes sense for IT8512A+ current measurement accuracy
                            #   to speed up.  Was 0.02 during testing.

import sys, math, random, os.path, time, dcload
        ...
        if volts > 4:                 # Start tracking when Voc > ...
            ...
            while True:
                ...
                if delta_watts == 0 or delta_volts == 0:
                    ...
                    # we are at a higher voltage level than Vmpp
                    Vset = Vset - set_volts_delta
                    direction_new = -1
                elif delta_watts/delta_volts > 0:
                    # we are at a lower voltage level than Vmpp
                    Vset = Vset + set_volts_delta
                    direction_new = 1
